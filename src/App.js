...
export default function PolicyPal() {
  ...
  if (file.type === "application/pdf") {
    ...
    reader.onload = (e) => { setDocText(e.target.result); setMessages([]); };
      ...
      <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', sans-serif", display: "flex", flexDirection: "column" }}>
        ...
        <header style={{ background: C.surface, borderBottom: `1px solid ${C.borderLight}`, padding: "1.1rem 2.5rem" }}>
          <div style={{ maxWidth: "700px", margin: "0 auto", display: "flex", alignItems: "center", gap: "1rem" }}>
            <div style={{ width: "38px", height: "38px", border: `1.5px solid ${C.gold}`, borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", color: C.gold, fontSize: "1rem", flexShrink: 0 }}>
              ◈
            </div>
            <div style={{ flex: 1 }}>
              <h1 style={{ fontFamily: "'Libre Baskerville', serif", fontSize: "1.25rem", fontWeight: "700", color: C.text, letterSpacing: "0.01em" }}>
                PolicyPal
              </h1>
              <p style={{ fontSize: "0.65rem", color: C.mutedLight, letterSpacing: "0.18em", textTransform: "uppercase", marginTop: "1px" }}>
                HR Policy Assistant · by Divyah
              </p>
            </div>
            {docName && (
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                <span style={{ fontSize: "0.72rem", color: C.gold, fontWeight: "500" }}>
                  {docName.slice(0, 22)}{docName.length > 22 ? "…" : ""}
                </span>
                <button className="change-btn" onClick={() => { setDocText(""); setDocName(""); setMessages([]); }} style={{ background: "none", border: "none", color: C.muted, fontSize: "0.7rem", cursor: "pointer", letterSpacing: "0.08em", textDecoration: "underline", textUnderlineOffset: "3px", transition: "color 0.2s" }}>
                  change
                </button>
              </div>
            )}
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "2.5rem", paddingBottom: "5rem" }}>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ fontSize: "0.85rem", color: C.muted, lineHeight: 1.8, maxWidth: "380px", fontWeight: "300" }}>
                  Upload any HR policy document. Ask questions in plain language. Get precise answers instantly.
                </p>
              </div>
              ...
              >
                ...
                <div style={{ fontFamily: "'Libre Baskerville', serif", fontSize: "1.05rem", color: C.text, marginBottom: "0.5rem" }}>
                  Drop your document here
                </div>
                <div style={{ fontSize: "0.75rem", color: C.muted, marginBottom: "1.75rem", lineHeight: 1.7, fontWeight: "300" }}>
                  .txt or .md files · drag & drop or browse
                </div>
                <button style={{ background: C.text, color: C.surface, border: "none", borderRadius: "8px", padding: "0.7rem 1.75rem", fontSize: "0.73rem", fontWeight: "500", cursor: "pointer", letterSpacing: "0.1em", textTransform: "uppercase", transition: "background 0.2s" }}>
                  Choose File
                </button>
              ...
              <div style={{ flex: 1, overflowY: "auto", paddingBottom: "1rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                ...
                {messages.length === 0 && (
                  <div className="msg" style={{ background: C.surface, border: `1px solid ${C.borderLight}`, borderRadius: "16px", padding: "1.75rem" }}>
                    ...
                    <p style={{ fontSize: "0.8rem", color: C.muted, lineHeight: 1.8, marginBottom: "1.1rem", fontWeight: "300" }}>
                      I've read your policy document carefully. Here are some things people typically ask:
                    </p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                      {SUGGESTIONS.map((s, i) => (
                        <button key={i} className="sug" onClick={() => sendMessage(s)} style={{ background: C.surfaceAlt, border: `1px solid ${C.borderLight}`, borderRadius: "8px", padding: "0.7rem 0.9rem", fontSize: "0.73rem", color: C.muted, cursor: "pointer", textAlign: "left", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s", lineHeight: 1.45 }}>
                          {s}
                        </button>
                      ))}
                    </div>
                ...
                {messages.map((m, i) => (
                  <div key={i} className="msg" style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", gap: "0.6rem", alignItems: "flex-end" }}>
                    ...
                    <div style={{
                      ...
                      color: m.role === "user" ? C.bg : C.textSoft,
                      fontSize: "0.82rem",
                      lineHeight: "1.7",
